...
void Q11()
{
    ...
    for(int j = 0; j < 20; j++)
    {
        for(int i = 0; i < 16; i++)
        {
            Down = M[i][j] * M[i][j+1] * M[i][j+2] * M[i][j+3];                  //Down
            if(Down > Max)
                {
                    Max = Down;
                }
            ...
            if(i < 16)                                                          //Left
            {
                Left = M[i][j] * M[i+1][j] * M[i+2][j] * M[i+3][j];
                if(Left > Max)
                {
                    Max = Left;
                }
                DownLeft = M[i][j] * M[i+1][j+1] * M[i+2][j+2] * M[i+3][j+3];    //Down-Left
                if(DownRight > Max)
                {
                    Max = DownRight;
                }
            ...
            if(i > 3)
            {                                                                   //Down-Right
                DownRight = M[i][j] * M[i+1][j-1] * M[i+2][j-2] * M[i+3][j-3];
                if(DownRight > Max)
                {
                    Max = DownRight;
                }
